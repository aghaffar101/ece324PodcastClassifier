...
if __name__ == "__main__":
    ...
    epochsLis = np.arange(num_epochs,1)
    trainLossLis = np.empty(shape=(epochsLis.shape[0]))
    testLossLis = np.empty(shape=(epochsLis.shape[0]))
    num_epochs = 5
    for epoch in range(num_epochs):
        train_loss = train(model, train_dataloader, device)
        test_loss = test(model, test_dataloader, device)
        print(f"Epoch: {epoch+1}, Train oss: {train_loss:.4f}, Test Loss: {test_loss}")
        trainLossLis[epoch] = 
