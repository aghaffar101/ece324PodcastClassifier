...
def train(model, dataloader, device, lossFcn):
    model.train()
    running_loss = 0.0
...
def test(model, dataloader, device, lossFcn):
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        train_loss = train(model, train_dataloader, device, lossFcn)
        test_loss = test(model, test_dataloader, device, lossFcn)
